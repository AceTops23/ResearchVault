...
def generate_abstract():
    ...
    if record is not None:
        ...
        # Add this line to remove special characters and numbers
        cleaned_text = re.sub(r'\W', ' ', cleaned_text)
        
        chunk_size = 512
        text_chunks = [cleaned_text[i:i+chunk_size] for i in range(0, len(cleaned_text), chunk_size)]
        
        # Initialize each section with the first chunk
        section_texts = {section: [(text_chunks[0], 0)] for section in section_order}
        ...
        for i in range(0, len(text_chunks), batch_size):
            ...
            inputs = vectorizer.transform(batch)
            
            predicted_sections = model.predict(inputs)
            predicted_probs = model.predict_proba(inputs)
            
            for j, text_chunk in enumerate(batch):
                current_section = section_names[predicted_sections[j]]
                if current_section in section_texts:
                    section_texts[current_section].append((text_chunk, np.max(predicted_probs[j])))
                
            print(f"Finished classifying batch {i//batch_size + 1}.")
            
        # Process each section
        for section in section_order:
            print(f"Processing section {section}...")
            
            # Sort chunks by probability and select top 5
            top_chunks = sorted(section_texts[section], key=lambda x: x[1], reverse=True)[:5]
            
            # Check if top_chunks is not empty
            if top_chunks:
                # Randomly select a chunk from the top 5
                selected_chunk = random.choice(top_chunks)
                
                # Replace the list of chunks with the selected chunk's text
                section_texts[section] = selected_chunk[0]
                
                print(f"Selected chunk for {section} with probability {selected_chunk[1]}")
            else:
                print(f"No chunks found for {section}.")
        ...
        ordered_section_texts = {section: f"{section}: {section_texts[section]}" for section in section_order}
        
        return jsonify(ordered_section_texts)
